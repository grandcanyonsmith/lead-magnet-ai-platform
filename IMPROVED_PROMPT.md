# Improved Prompt: Upload Template HTML with Data JSON

## Task Description

Download the HTML template from S3, create a new report instance with a unique identifier, and upload both the HTML file and associated JSON data file.

## Steps

1. **Download the source template**
   - Source: `s3://cc360-pages/templates/market-research-report/v4.html`
   - Region: `us-west-2`
   - Bucket: `cc360-pages`

2. **Generate a unique identifier**
   - Use UUID v4 format (e.g., `d112f1b8-6592-4870-90ff-c73c76547305`)
...
**IMPORTANT:** The `data.json` file should contain the JSON structure below, but **all values must be customized based on the specific course topic** provided. The example values shown here (Dragonflight Academy, dragon training, etc.) are just a template—you must replace them with values relevant to the actual course topic being processed.

The `data.json` file should contain the following structure (provided as a JSON object):
...
**Note:** The above JSON shows example values for a "Dragon Training" course. When processing a different course topic, you must:
- Replace all brand names, product names, and course-specific terminology
- Update all descriptions, features, pains, desires, and other content to match the actual course topic
- Keep the same JSON structure and key names, but change all values to be relevant to the course being processed
- Ensure all content is coherent and appropriate for the specific course topic
...
- If the source template cannot be downloaded, return an error message
- If either upload fails, return an error message with details
- Validate that the JSON data is valid before uploading
