...
lastUpdated: 2025-12-18T23:02:29.000Z
...
<Properties>
  ...
  * `boolean`
  ...
  Indicates whether or not sign in attempts are transferable to the sign up flow. Defaults to `true`. When set to `false`, prevents <Tooltip><TooltipTrigger>opaque sign ups</TooltipTrigger><TooltipContent>An **opaque sign up** refers to a specific OAuth authentication flow where a user attempts to sign in via OAuth (like Google, GitHub, etc.) with an email address that doesn't exist in your application's user database. The term "opaque" here means that the sign-up process is transparent to the user - they think they're signing in, but Clerk is actually <Tooltip><TooltipTrigger>creating a new account behind the scenes</TooltipTrigger><TooltipContent>A **transfer flow** allows a user to both sign in and sign up in the same process. If a user tries signing up, but already exists, the flow will transfer the user to the sign-in flow. If a user tries signing in, but doesn't exist, the flow will transfer the user to the sign-up flow.</TooltipContent></Tooltip>.</TooltipContent></Tooltip> when a user attempts to sign in via OAuth with an email that doesn't exist.
