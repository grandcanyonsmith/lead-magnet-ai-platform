...
export function createResourceKeys(resourceName: string) {
  return {
    ...
    list: (params?: Record<string, unknown>) =>
      [...createResourceKeys(resourceName).lists(), params] as const,
    details: () => [...createResourceKeys(resourceName).all, "detail"] as const,
    detail: (id: string) =>
      [...createResourceKeys(resourceName).details(), id] as const,
  };
}
...
export function createResourceHooks<
  TItem,
  TListResponse extends Record<string, TItem[]>,
  TCreateRequest,
  TUpdateRequest,
>(
  ...
  function useCreate() {
    ...
    >((data: TCreateRequest) => config.api.create(data), {
      showSuccessToast:
        messages.create || `${config.resourceName} created successfully`,
      showErrorToast: true,
  ...
  function useUpdate() {
    ...
    >(({ id, data }) => config.api.update(id, data), {
      showSuccessToast:
        messages.update || `${config.resourceName} updated successfully`,
      showErrorToast: true,
      invalidateQueries: [resourceKeys.all],
    });
  ...
  function useDelete() {
    const { mutateAsync, isPending, error } = useMutation<void, Error, string>(
      ...
      {
        showSuccessToast:
          messages.delete || `${config.resourceName} deleted successfully`,
        showErrorToast: true,
        invalidateQueries: [resourceKeys.all],
      },
    );
