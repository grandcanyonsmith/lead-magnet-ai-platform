...
const TOOL_ICONS: Record<string, { icon: typeof FiZap; label: string; tint: string }> = {
  ...
  code_interpreter: { icon: FiCode, label: 'Code Interpreter', tint: 'bg-slate-100 text-slate-600' },
}
  ...
  return (
    <div
      ...
      className={[
        ...
        isDropTarget ? 'border-dashed border-primary-400 shadow-[0_0_35px_-20px,#2563eb]' : '',
        isDragging ? 'scale-[1.02] shadow-xl shadow-primary-200/60 opacity-95' : '',
        animateIn ? 'flow-node-animate-in' : '',
      ].join(' ')}
    >
      <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${modelStyle.accent} to-white opacity-70`} />

      <Handle type="target" position={Position.Left} className="h-3 w-3 rounded-full border-2 border-white bg-primary-500" />
      ...
      <div className="relative z-10 space-y-3">
        ...
        <div className="space-y-1">
          <h3 className="line-clamp-2 text-base font-semibold text-slate-900">{step.step_name || `Step ${index + 1}`}</h3>
          <p className="line-clamp-2 text-sm text-slate-500">{step.step_description || 'No description provided yet.'}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {step.tool_choice && step.tool_choice !== 'none' && (
            <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2 py-0.5 text-[11px] font-medium text-primary-700">
              {step.tool_choice === 'required' ? 'Tools Required' : 'Tools Auto'}
            </span>
          )}
        </div>
        ...
        {hasTools && (
          <div className="flex flex-wrap items-center gap-2 rounded-xl bg-slate-50/80 px-3 py-2 text-[11px] text-slate-500">
            {tools.slice(0, 3).map((tool, idx) => {
              ...
              const toolMeta = TOOL_ICONS[toolKey] || { icon: FiZap, label: toolKey, tint: 'bg-slate-100 text-slate-600' }
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400">
          <span>{step.instructions?.trim() ? `${step.instructions.trim().split(' ').length} words` : 'No instructions yet'}</span>
          <span>{step.tools?.length ? `${step.tools.length} tools` : 'No tools'}</span>
        </div>
      ...
      <Handle type="source" position={Position.Right} className="h-3 w-3 rounded-full border-2 border-white bg-primary-500" />
    </div>
  )
}
