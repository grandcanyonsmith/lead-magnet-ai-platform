...
export function ChatInterface({
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="px-3 py-1.5 text-xs rounded-lg border border-border text-muted-foreground hover:border-emerald-300 dark:hover:border-emerald-700 hover:text-emerald-700 dark:hover:text-emerald-200 transition-colors"
            >
              Edit context
            </button>
            <button
              ...
              className="px-3 py-1.5 text-xs rounded-lg border border-border text-muted-foreground hover:bg-accent/40 hover:text-foreground transition-colors"
            >
              Reset chat
            </button>
          </div>
        ...
        {contextItems.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {contextItems.map((item) => (
              <div
                key={item.label}
                className="rounded-lg border border-border/60 bg-muted/50 px-2.5 py-2 max-w-full"
      ...
      <div
        ref={chatScrollRef}
        className="rounded-lg border border-border bg-gradient-to-b from-gray-50/50 to-white dark:from-gray-900/50 dark:to-gray-950 p-4 max-h-[500px] overflow-y-auto space-y-4 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700"
      >
            ...
            <div key={message.id} className="space-y-3">
                ...
                >
                  {isUserMessage ? (
                    ...
                    <MarkdownRenderer
                      ...
                      fallbackClassName="whitespace-pre-wrap text-sm text-foreground"
                    />
                  )}
                </div>
              </div>
              ...
              {message.deliverables && message.deliverables.length > 0 && (
                <div className="flex justify-start animate-in fade-in slide-in-from-bottom-3 duration-300">
                  <div className="w-full rounded-2xl border border-emerald-200/70 dark:border-emerald-900/60 bg-card px-4 py-4 shadow-sm">
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 pt-3">
                      {message.deliverables.map((deliverable, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div className="relative aspect-[4/3] rounded-lg overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 border border-border mb-3 group-hover:scale-[1.02] transition-transform duration-200">
                            ...
                            <div className="space-y-2">
                              ...
                              {deliverable.build_description?.trim() && (
                                <div className="rounded-lg bg-muted/60 border border-border/60 px-2 py-1.5 space-y-1">
                                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">
                                    Build plan
                                  </div>
                              ...
                              <div className="flex items-center gap-2 pt-1">
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground uppercase tracking-wide font-medium">
                                        ...
                                        .map((image, imageIndex) => {
                                          ...
                                          return (
                                            <a
                                              ...
                                              target="_blank"
                                              rel="noreferrer"
                                              className="relative h-12 w-12 rounded-md overflow-hidden border border-border"
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {CHAT_STARTER_PROMPTS.map((prompt) => (
                <button
                  ...
                  className="text-left p-3 rounded-lg border border-border hover:border-emerald-300 dark:hover:border-emerald-700 bg-card hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all group"
                >
                  <div className="text-sm font-semibold text-foreground group-hover:text-emerald-700 dark:group-hover:text-emerald-300">
                    {prompt.title}
                  </div>
      ...
      <div className="space-y-4">
        <div className="relative">
          <textarea
            ...
            disabled={isIdeating}
            className="w-full pl-4 pr-12 py-3 rounded-xl border-2 border-border bg-card text-foreground placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:focus:ring-emerald-400 focus:border-transparent resize-none shadow-sm"
            rows={3}
          />
          <button
