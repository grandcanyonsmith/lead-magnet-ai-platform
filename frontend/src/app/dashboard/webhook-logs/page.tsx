...
export default function WebhookLogsPage() {
  ...
  const handleRetry = async (log: WebhookLog) => {
    ...
    try {
      ...
      toast.error(
        `Failed to retry webhook: ${error.message || "Unknown error"}`,
      );
    } finally {
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-0">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600 mt-1">
              View and retry webhook requests
            </p>
          </div>
          <button
      ...
      {logs.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-6 sm:p-12 text-center">
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-2">
            Webhook requests will appear here once they are received
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-lg shadow overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                        ...
                        {log.processing_time_ms && (
                          <span>
                            Duration: {formatDuration(log.processing_time_ms)}
                          </span>
                        )}
                        {log.response_status && (
