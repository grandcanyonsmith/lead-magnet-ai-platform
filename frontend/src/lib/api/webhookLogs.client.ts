...
export class WebhookLogsClient extends BaseApiClient {
  ...
  async getWebhookLogs(
    ...
    if (params?.webhook_token)
      queryParams.append("webhook_token", params.webhook_token);
  ...
  async retryWebhook(logId: string): Promise<{ message: string; result: any }> {
    return this.post<{ message: string; result: any }>(
      `/admin/webhook-logs/${logId}/retry`,
    );
  }
}
