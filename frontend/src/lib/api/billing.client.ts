...
export class BillingClient extends BaseApiClient {
  ...
  async getSubscription(): Promise<SubscriptionInfo> {
    const response = await this.get<SubscriptionInfo>('/admin/billing/subscription')
    return response
  }
  ...
  async createCheckoutSession(
    ...
    const response = await this.post<CheckoutSessionResponse>('/admin/billing/checkout-session', {
  ...
  async createPortalSession(returnUrl?: string): Promise<PortalSessionResponse> {
    const response = await this.post<PortalSessionResponse>('/admin/billing/portal-session', {
