...
import { Settings, SettingsUpdateRequest } from '@/shared/types'
...
export const settingsKeys = {
  ...
  usage: (startDate?: string, endDate?: string) => [...settingsKeys.all, 'usage', startDate, endDate] as const,
}
...
export function useUpdateSettings(): UseUpdateSettingsResult {
  const { mutateAsync, isPending, error } = useMutation<Settings, Error, SettingsUpdateRequest>(
...
export function useUsage(startDate?: string, endDate?: string): UseUsageResult {
  const queryKey = useMemo(() => settingsKeys.usage(startDate, endDate), [startDate, endDate])
