import { FiCopy, FiRefreshCw, FiChevronDown, FiChevronUp } from 'react-icons/fi'
    ...
    return (
      <div className="mt-1">
        ...
        >
          ...
          <span className="text-xs font-medium">Array ({value.length} items)</span>
        </button>
        {expanded && (
  ...
  if (typeof value === 'object') {
    ...
    return (
      <div className="mt-1">
        ...
        >
          {expanded ? <FiChevronUp className="w-3 h-3" /> : <FiChevronDown className="w-3 h-3" />}
          <span className="text-xs font-medium">Object ({entries.length} fields)</span>
        </button>
        {expanded && (
          <div className="ml-4 mt-1 space-y-2 border-l-2 border-gray-200 pl-3">
            {entries.map(([k, v]) => (
...
export function SubmissionSummary({ submission, onResubmit, resubmitting }: SubmissionSummaryProps) {
  ...
  const nameField = entries.find(([key]) => 
    ['name', 'full_name', 'first_name', 'submitter_name'].includes(key.toLowerCase())
  )
    ...
    <section className="mb-4 sm:mb-6">
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div className="flex flex-col gap-3 p-4 sm:p-6 md:flex-row md:items-center md:justify-between bg-gradient-to-r from-gray-50 to-white">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-1">Form Submission</p>
