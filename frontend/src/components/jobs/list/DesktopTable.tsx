...
export function JobsDesktopTable({
  ...
  return (
    <div className="hidden md:block bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden" data-tour="jobs-list">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50/50">
          <tr>
            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Lead Magnet</th>
            ...
            <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Output</th>
        ...
        <tbody className="bg-white divide-y divide-gray-100">
          {jobs.map((job) => {
            ...
            const errorPreview = hasError && job.error_message
              ? (job.error_message.length > 80 ? `${job.error_message.substring(0, 80)}...` : job.error_message)
              : null
            ...
            return (
              <React.Fragment key={job.job_id}>
                <tr 
                  className={clsx(
                    "group hover:bg-gray-50/50 cursor-pointer transition-all duration-150",
                ...
                >
                  <td className="px-6 py-4">
                    ...
                    {submissionPreview && (
                      <div className="text-xs text-gray-500 mt-1 font-medium truncate max-w-xs">
                        {submissionPreview}
                      </div>
                    )}
                    {stepMeta.label && <div className="text-[11px] text-gray-400 mt-0.5">{stepMeta.label}</div>}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-700">{formatRelativeTime(job.created_at)}</div>
                    <div className="text-[11px] text-gray-400 font-medium">{new Date(job.created_at).toLocaleDateString()}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap" data-tour="view-artifacts">
                    {job.output_url ? (
                      <button
                        type="button"
                        onClick={(e) => {
                        ...
                        className="inline-flex items-center gap-1.5 text-primary-600 hover:text-primary-700 text-xs font-bold transition-colors disabled:opacity-50"
                      >
                        {isOpening ? (
                      ...
                      <span className="text-xs font-bold text-gray-300">No asset</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <ChevronRightIcon className="h-5 w-5 text-gray-300 group-hover:text-primary-400 transition-colors inline-block" />
                  </td>
                </tr>
                {hasError && (
                  <tr
                    key={`${job.job_id}-error`}
                    className="bg-red-50/50 hover:bg-red-100/50 cursor-pointer transition-all duration-150 border-t-0"
                    onClick={() => onNavigate(job.job_id)}
                  >
                    <td colSpan={6} className="px-6 py-3">
                      <div className="flex items-start bg-white/50 rounded-lg p-3 border border-red-100">
                        <XCircleIcon className="w-5 h-5 text-red-500 mt-0.5 mr-3 flex-shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-red-800 uppercase tracking-tight">Generation failed</p>
                          <p className="text-sm text-red-700 mt-1 leading-relaxed font-medium line-clamp-2">{errorPreview}</p>
