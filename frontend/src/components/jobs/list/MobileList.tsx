...
import { ArrowTopRightOnSquareIcon, ArrowPathIcon, ChevronRightIcon, XCircleIcon } from '@heroicons/react/24/outline'
          ...
          >
            <div className="p-4">
                <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <div className="shrink-0 flex flex-col items-end gap-1.5" data-tour="job-status">
                ...
                <div className="flex items-center justify-between py-3 border-y border-gray-50">
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[11px] font-bold text-gray-400 uppercase tracking-tight">Started</span>
                        <span className="text-xs font-medium text-gray-700">{formatRelativeTime(job.created_at)}</span>
                    </div>
                    <div className="flex flex-col items-end gap-0.5 text-right">
                        <span className="text-[11px] font-bold text-gray-400 uppercase tracking-tight">Duration</span>
                        <span className="text-xs font-medium text-gray-700">{duration !== null ? formatDuration(duration) : '—'}</span>
                    </div>
                </div>

                <div className="mt-3 flex items-center justify-between gap-4">
                    {job.output_url ? (
                        <div data-tour="view-artifacts" onClick={(e) => e.stopPropagation()}>
                            <button
                                type="button"
                ...
                {hasError && (
                    <div className="mt-4 p-3 rounded-lg bg-red-50 border border-red-100 flex items-start gap-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                            <p className="text-[11px] font-bold text-red-800 uppercase tracking-tight">Error</p>
                            <p className="text-xs font-medium text-red-700 mt-0.5 line-clamp-2 leading-relaxed">{job.error_message}</p>
                        </div>
                    </div>
                )}
            </div>
