...
function getAuthToken(): string | null {
  ...
  token =
    localStorage.getItem(`CognitoIdentityServiceProvider.${clientId}.${lastAuthUser}.accessToken`) ||
...
export function reportReactError(error: Error, errorInfo: ErrorInfo): void {
  ...
  try {
    ...
    if (selectedCustomerId) headers['X-Selected-Customer-Id'] = selectedCustomerId
