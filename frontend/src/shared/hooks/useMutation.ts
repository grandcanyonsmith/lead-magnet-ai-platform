...
export interface UseMutationOptions<TData, TError = Error, TVariables = void> extends Omit<ReactUseMutationOptions<TData, TError, TVariables>, 'mutationFn'> {
...
export function useMutation<TData = unknown, TError = Error, TVariables = void>(
  ...
  const { showSuccessToast, showErrorToast, invalidateQueries, onSuccess, onError, ...mutationOptions } = options || {}
  ...
  return useReactMutation<TData, TError, TVariables>({
    ...
    onSuccess: async (data, variables, context) => {
      ...
      if (showSuccessToast) {
        const message = typeof showSuccessToast === 'string' ? showSuccessToast : 'Operation completed successfully'
        toast.success(message)
      }

      // Invalidate related queries if specified
      if (invalidateQueries) {
        const { queryClient } = await import('@/shared/lib/react-query')
