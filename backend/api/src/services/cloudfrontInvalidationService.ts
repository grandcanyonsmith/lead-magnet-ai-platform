...
export async function invalidateCloudFrontPaths(
  ...
  if (!distributionId) {
    logger.warn(
      "[CloudFrontInvalidation] CLOUDFRONT_DISTRIBUTION_ID not set; skipping invalidation",
      {
  ...
  try {
    ...
    logger.warn(
      "[CloudFrontInvalidation] aws-sdk not available; skipping invalidation",
      {
  ...
  if (!CloudFront) {
    logger.warn(
      "[CloudFrontInvalidation] aws-sdk CloudFront client not found; skipping invalidation",
    );
    return;
  }
