...
export class ShellAbuseControlService {
  ...
  async consumeIpToken(args: {
    ...
    const limitPerHour = Number.isFinite(args.limitPerHour) ? Math.max(1, Math.floor(args.limitPerHour)) : 10;
    ...
    try {
      ...
      if (error?.name === 'ConditionalCheckFailedException') {
        logger.warn('[ShellAbuseControl] IP rate limit exceeded', { sourceIp, limitPerHour, bucket });
        throw new ApiError('Rate limit exceeded. Please try again later.', 429);
      }
      logger.error('[ShellAbuseControl] Failed to apply IP rate limiting (fail closed)', {
        sourceIp,
        error: error?.message || String(error),
      });
      throw new ApiError('Rate limiting unavailable. Please try again later.', 503);
    }
  }
  ...
  async acquireGlobalSlot(args: {
    ...
    const tableName = env.rateLimitsTable;
    const maxInFlight = Number.isFinite(args.maxInFlight) ? Math.max(1, Math.floor(args.maxInFlight)) : 5;
    const waitMs = Number.isFinite(args.waitMs) ? Math.max(0, Math.floor(args.waitMs)) : 0;
      ...
      try {
        await docClient.send(new UpdateCommand({
          ...
          UpdateExpression:
            ...
            '#updated_at = :now, ' +
            '#created_at = if_not_exists(#created_at, :now)',
          ConditionExpression: 'attribute_not_exists(#in_flight) OR #in_flight < :limit',
          ExpressionAttributeNames: {
            '#in_flight': 'in_flight',
            '#ttl': 'ttl',
            '#created_at': 'created_at',
        ...
        throw new ApiError('Concurrency limiter unavailable. Please try again later.', 503);
      }
    };
    ...
    while (true) {
          ...
          release: async () => {
            try {
              ...
              await docClient.send(new UpdateCommand({
                TableName: tableName,
                Key: { pk },
                UpdateExpression: 'ADD #in_flight :dec SET #ttl = :ttl, #updated_at = :now',
                ConditionExpression: 'attribute_exists(#in_flight) AND #in_flight >= :one',
                ExpressionAttributeNames: {
                  '#in_flight': 'in_flight',
                  '#ttl': 'ttl',
                  '#updated_at': 'updated_at',
              ...
              logger.warn('[ShellAbuseControl] Failed to release global slot (leak-protected by TTL)', {
                error: error?.message || String(error),
              });
            }
          },
      ...
      const sleepMs = Math.min(500, Math.max(50, Math.floor(remaining / 5))) + Math.floor(Math.random() * 25);
      await delay(sleepMs);
    }
  }
}
