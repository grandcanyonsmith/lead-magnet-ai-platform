...
export class RateLimitService {
  async consumeFormSubmissionToken(args: {
    ...
    if (!tableName || tableName.trim().length === 0) {
      ...
      logger.warn('[RateLimitService] RATE_LIMITS_TABLE not configured; skipping rate limiting', {
        tenantId,
        formId,
      });
    ...
    try {
      ...
      logger.error('[RateLimitService] Failed to apply rate limiting (fail open)', {
        tenantId,
        formId,
        sourceIp,
