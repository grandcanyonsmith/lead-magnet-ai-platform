...
export function requireUser(context?: RequestContext): AuthContext {
  if (!context) {
    ...
    throw new AuthenticationError(
      "Request context is missing. This may indicate a configuration issue.",
      {
        message:
          "The request context was not provided. This could indicate a problem with the API gateway or middleware configuration.",
      },
    );
  }
  ...
  if (!context.auth) {
    ...
    throw new AuthenticationError(
      "Please sign in to access this resource. If you are already signed in, your session may have expired or there may be an issue with your account configuration.",
      {
  ...
  if (auth.role !== "SUPER_ADMIN") {
    ...
    throw new AuthorizationError(
      `You do not have permission to access this resource. Required role: SUPER_ADMIN, but your role is: ${auth.role || "not set"}`,
      {
        ...
        customerId: auth.customerId,
        troubleshooting:
          'If you believe you should have superadmin access, verify that your user record in the database has role="SUPER_ADMIN"',
      },
    );
  }
...
export function getCustomerId(context?: RequestContext): string {
  ...
  if (!auth.customerId) {
    logger.error(
      "[RBAC.getCustomerId] Customer ID is missing from auth context",
      {
        userId: auth.actingUserId,
        role: auth.role,
    ...
    throw new ApiError(
      ...
      "MISSING_CUSTOMER_ID",
      {
        ...
        role: auth.role,
        troubleshooting:
          "For superadmin accounts, verify that the customer_id field is properly set in the user database record",
      },
    );
  }
