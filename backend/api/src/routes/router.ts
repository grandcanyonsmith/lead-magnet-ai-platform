...
class SimpleRouter {
   ...
   */
  register(method: string, path: string, handler: SimpleRouteHandler, requiresAuth: boolean = path.startsWith('/admin')): void {
   ...
   */
  async match(event: APIGatewayProxyEventV2, tenantId?: string): Promise<RouteResponse> {
