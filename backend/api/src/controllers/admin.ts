...
class AdminController {
  ...
  async updateUserRole(
    ...
    if (!role || !["USER", "ADMIN", "SUPER_ADMIN"].includes(role)) {
      ...
      throw new ApiError(
        "Valid role is required (USER, ADMIN, SUPER_ADMIN)",
        400,
      );
    }
    ...
    try {
      ...
      if (user.role === "SUPER_ADMIN" && role !== "SUPER_ADMIN") {
        ...
        if (superAdmins.length === 0) {
          logger.warn("[Admin] Attempted to demote last SUPER_ADMIN", {
            userId,
          });
      ...
      if (!updated) {
        ...
        throw new ApiError(
          "Failed to update user - database update returned no result",
          500,
        );
      }
      ...
      throw new ApiError(`Failed to update user role: ${errorMessage}`, 500);
    }
  }
