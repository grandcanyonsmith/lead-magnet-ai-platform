  ...
  async rerunStep(tenantId: string, jobId: string, stepIndex: number, continueAfter: boolean = false): Promise<RouteResponse> {
    ...
    try {
      if (env.isDevelopment() || !STEP_FUNCTIONS_ARN) {
        ...
        logger.info('[Job Rerun Controller] Executing worker command for step rerun', {
          jobId,
          stepIndex,
          continueAfter,
          command,
        ...
        try {
          ...
          logger.info('[Job Rerun Controller] Step rerun completed', { jobId, stepIndex, stdout, stderr });
          
          return {
            statusCode: 200,
            body: { 
              message: continueAfter ? 'Step rerun and continue initiated' : 'Step rerun initiated', 
              job_id: jobId, 
