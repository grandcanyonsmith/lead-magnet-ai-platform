...
class BillingController {
  async getUsage(
    ...
    try {
      ...
      if (
        ...
        logger.warn("[Billing] Usage records table not accessible", {
          ...
          suggestion:
            "Table needs to be created via CDK deployment and permissions granted",
        });
        usageRecords = [];
      } else {
        ...
        throw new ApiError(
          `Failed to fetch usage records: ${error.message}`,
          500,
        );
      }
    }
    ...
    summary.total_actual_cost =
      Math.round(summary.total_actual_cost * 1000000) / 1000000;
    summary.total_upcharge_cost =
      Math.round(summary.total_upcharge_cost * 1000000) / 1000000;
    ...
    Object.values(summary.by_service).forEach((service) => {
      ...
      service.upcharge_cost =
        Math.round(service.upcharge_cost * 1000000) / 1000000;
    });
  ...
  async createCheckoutSession(
    ...
    try {
      ...
      if (!customer.stripe_customer_id) {
        logger.info(
          "[Billing] Customer has no Stripe customer ID, creating one",
          { tenantId },
        );
        ...
        logger.info("[Billing] Created Stripe customer", {
          tenantId,
          stripeCustomerId,
        });
      }
      ...
      const successUrl =
        body.success_url || `${env.apiUrl}/setup-billing/success`;
  ...
  async createPortalSession(
    ...
    try {
      ...
      const returnUrl =
        body.return_url ||
        env.stripePortalReturnUrl ||
        `${env.apiUrl}/dashboard/settings?tab=billing`;
    ...
    try {
      ...
      if (subscription.current_period_start) {
        ...
        try {
          ...
          const usageRecords = normalizeQueryResult(
            usageRecordsResult,
          ) as UsageRecord[];
          ...
          usageSummary.total_actual_cost =
            Math.round(usageSummary.total_actual_cost * 1000000) / 1000000;
          usageSummary.total_upcharge_cost =
            Math.round(usageSummary.total_upcharge_cost * 1000000) / 1000000;
          Object.values(usageSummary.by_service).forEach((service) => {
            service.actual_cost =
              Math.round(service.actual_cost * 1000000) / 1000000;
            service.upcharge_cost =
              Math.round(service.upcharge_cost * 1000000) / 1000000;
          });
        } catch (error: any) {
