...
class WebhooksController {
  ...
  async handleWebhook(token: string, body: any, sourceIp: string): Promise<RouteResponse> {
    ...
    if (validatedBody.workflow_id) {
      workflow = await db.get(WORKFLOWS_TABLE, { workflow_id: validatedBody.workflow_id });
      ...
      if (workflow.tenant_id !== tenantId) {
        throw new ApiError('You don\'t have permission to access this workflow', 403);
      }
      ...
      workflow = workflows.find(
        (w: any) => w.workflow_name === validatedBody.workflow_name && !w.deleted_at
      );
      ...
      throw new ApiError('Either workflow_id or workflow_name is required', 400);
    }

    logger.info('[Webhooks] Found workflow', { workflowId: workflow.workflow_id });
    ...
    try {
      if (process.env.IS_LOCAL === 'true' || process.env.NODE_ENV === 'development' || !STEP_FUNCTIONS_ARN) {
        ...
        logger.info('[Webhooks] Started Step Functions execution', { jobId, workflowId: workflow.workflow_id });
      }
    } catch (error: any) {
