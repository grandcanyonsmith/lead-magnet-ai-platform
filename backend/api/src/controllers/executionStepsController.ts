...
export class ExecutionStepsController {
  ...
  async getExecutionSteps(
    ...
    try {
      const executionSteps = await executionStepsService.fetchFromS3(
      ...
      throw new ApiError(
        `Failed to fetch execution steps: ${error.message}`,
        500,
      );
    }
  }
  ...
  async quickEditStep(
    ...
    if (job.tenant_id !== tenantId) {
      throw new ApiError(
        "You don't have permission to edit steps for this job",
        403,
      );
    }
