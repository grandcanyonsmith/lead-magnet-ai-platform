...
if (!ARTIFACTS_TABLE) {
  logger.error(
    "[Artifacts Controller] ARTIFACTS_TABLE environment variable is not set",
  );
}
...
class ArtifactsController {
  async list(
        ...
        await Promise.all(
          Array.from(jobIds).map(async (jobId) => {
            try {
              ...
              logger.warn(`Failed to fetch workflow_id for job ${jobId}`, {
                error,
                job_id: jobId,
              });
            }
    ...
    const artifactsWithUrls = await Promise.all(
      artifacts.map(async (artifact: any) => {
        const workflowId = artifact.job_id
          ? jobIdToWorkflowId.get(artifact.job_id)
          : undefined;
        if (!artifact.s3_key) {
          return {
            ...artifact,
            ...
            size_bytes: artifact.file_size_bytes
              ? parseInt(artifact.file_size_bytes)
              : artifact.size_bytes,
        ...
        try {
          ...
          logger.error(
            `Error generating URL for artifact ${artifact.artifact_id}`,
            { error, artifact_id: artifact.artifact_id },
          );
          return {
            ...artifact,
            object_url: artifact.public_url || null,
  ...
  async getContent(
    ...
    if (artifact.tenant_id !== tenantId) {
      throw new ApiError(
        "You don't have permission to access this artifact",
        403,
      );
    }
    ...
    try {
      ...
      if (!response.Body) {
        throw new ApiError(
          `S3 object body is empty for key: ${artifact.s3_key}`,
          500,
        );
      }
      ...
      const contentType =
        response.ContentType || artifact.mime_type || "text/plain";
      ...
      if (
        ...
        throw new ApiError(
          `Artifact file not found in S3. Key: ${artifact.s3_key}`,
          404,
        );
      }
      ...
      throw new ApiError(
        `Failed to fetch artifact content: ${error.message}`,
        500,
      );
    }
  }
