...
/**
 * Auto-detect CloudFront domain from AWS CloudFormation
 * This improves the local dev experience by not requiring manual configuration
 */
async function getCloudFrontDomain() {
  ...
  try {
    ...
    if (output?.OutputValue) {
        console.log(`[Local Dev] Auto-detected CloudFront domain from AWS: ${output.OutputValue}`);
        return output.OutputValue;
    }
    return null;
  } catch (error) {
    // Don't spam warnings if just credentials are missing (common in some dev setups)
    if (error.name !== 'CredentialsProviderError') {
      console.warn(`[Local Dev] Could not auto-detect CloudFront domain: ${error.message}`);
    }
    return null;
  }
}
...
async function startServer() {
  ...
  // Use detected domain or fallback
  process.env.CLOUDFRONT_DOMAIN = cloudfrontDomain || process.env.CLOUDFRONT_DOMAIN || 'assets.mycoursecreator360.com';
