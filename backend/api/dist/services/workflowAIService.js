        ...
        if (workflowContext.current_steps.length === 0) {
            ...
            workflowContext.current_steps.forEach((step, index) => {
                const tools = step.tools && Array.isArray(step.tools) && step.tools.length > 0
                    ? ` [Tools: ${step.tools.map((t) => typeof t === 'string' ? t : (t?.type || 'unknown')).join(', ')}]`
                    : '';
                const deps = step.depends_on && Array.isArray(step.depends_on) && step.depends_on.length > 0
                    ...
                    : '';
                contextParts.push(`${index + 1}. ${step.step_name} (${step.model})${tools}${deps}`);
...
Please generate the updated workflow configuration with all necessary changes.`;
        logger_1.logger.info('[WorkflowAI] Editing workflow', {
        ...
        try {
            ...
            const validatedSteps = parsedResponse.steps.map((step, index) => {
                ...
                if (!AVAILABLE_MODELS.includes(step.model)) {
                    logger_1.logger.warn(`[WorkflowAI] Invalid model ${step.model}, defaulting to gpt-5`);
                    step.model = 'gpt-5';
                }
                // Validate and sanitize tools
                if (step.tools && Array.isArray(step.tools)) {
                    step.tools = step.tools.filter((tool) => {
                ...
                if (step.depends_on) {
                    step.depends_on = step.depends_on.filter((dep) => typeof dep === 'number' &&
                        dep >= 0 &&
            ...
            const normalizedSteps = (0, workflowMigration_1.ensureStepDefaults)(validatedSteps);
            // Validate dependencies across all steps
            try {
                (0, dependencyResolver_1.validateDependencies)(normalizedSteps);
            }
            catch (error) {
                logger_1.logger.error('[WorkflowAI] Dependency validation failed', { error: error.message });
                throw new Error(`Invalid step dependencies: ${error.message}`);
            }
            parsedResponse.steps = normalizedSteps;
            logger_1.logger.info('[WorkflowAI] Workflow edited successfully', {
            ...
            logger_1.logger.error('[WorkflowAI] Error editing workflow', {
                error: error.message,
                stack: error.stack,
            });
...
//# sourceMappingURL=workflowAIService.js.map