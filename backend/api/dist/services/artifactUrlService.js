...
exports.ArtifactUrlService = void 0;
...
const CLOUDFRONT_DOMAIN = env_1.env.cloudfrontDomain;
if (!ARTIFACTS_TABLE) {
    logger_1.logger.error('[ArtifactUrlService] ARTIFACTS_TABLE environment variable is not set');
}
if (!ARTIFACTS_BUCKET) {
    logger_1.logger.error('[ArtifactUrlService] ARTIFACTS_BUCKET environment variable is not set');
}
const s3Client = new client_s3_1.S3Client({ region: env_1.env.awsRegion });
const AWS_REGION = env_1.env.awsRegion;
class ArtifactUrlService {
    /**
     * Check if a URL is a presigned S3 URL (contains X-Amz- query parameters)
     */
    static isPresignedUrl(url) {
        if (!url)
            return false;
        try {
     ...
     */
    static async generatePresignedUrl(s3Key, expiresIn = 604800) {
        if (!ARTIFACTS_BUCKET) {
            throw new Error('ARTIFACTS_BUCKET is not configured');
        }
     ...
     */
    static needsUrlRefresh(artifact) {
        if (!artifact.s3_key)
            return false;
     ...
     */
    static async generateUrl(s3Key, artifact) {
     ...
     */
    static async refreshUrl(artifact) {
        if (!ARTIFACTS_TABLE) {
        ...
        await db_1.db.update(ARTIFACTS_TABLE, { artifact_id: artifact.artifact_id }, updateData);
        return { url, expiresAt };
    }
    /**
     * Ensure artifact has a valid URL (refresh if needed)
     */
    static async ensureValidUrl(artifact) {
        if (!artifact.s3_key) {
...
exports.ArtifactUrlService = ArtifactUrlService;
//# sourceMappingURL=artifactUrlService.js.map