...
exports.WorkflowStepAIService = void 0;
        ...
        if (workflowContext.current_steps.length === 0) {
            ...
            workflowContext.current_steps.forEach((step, index) => {
                const tools = step.tools && step.tools.length > 0
                    ...
                    : '';
                contextParts.push(`${index + 1}. ${step.step_name} (${step.model})${tools}${step.step_description ? ` - ${step.step_description}` : ''}`);
            });
        }
...
Please generate the workflow step configuration.`;
        logger_1.logger.info('[WorkflowStepAI] Generating step', {
            workflow: workflowContext.workflow_name,
        ...
        try {
            ...
            if (!AVAILABLE_MODELS.includes(parsedResponse.step.model)) {
                logger_1.logger.warn(`[WorkflowStepAI] Invalid model ${parsedResponse.step.model}, defaulting to gpt-5`);
                parsedResponse.step.model = 'gpt-5';
            }
            // Validate and sanitize tools
            if (parsedResponse.step.tools) {
                parsedResponse.step.tools = parsedResponse.step.tools.filter((tool) => {
            ...
            if (parsedResponse.action === 'update' && currentStep) {
                ...
                if (parsedResponse.step.depends_on === undefined || parsedResponse.step.depends_on === null) {
                    ...
                    logger_1.logger.info('[WorkflowStepAI] Preserved existing dependencies', {
            ...
            logger_1.logger.info('[WorkflowStepAI] Step generated successfully', {
            ...
            logger_1.logger.error('[WorkflowStepAI] Error generating step', {
                error: error.message,
                stack: error.stack,
            });
...
exports.WorkflowStepAIService = WorkflowStepAIService;
//# sourceMappingURL=workflowStepAIService.js.map