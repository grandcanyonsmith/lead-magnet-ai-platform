...
exports.TemplateGenerationService = void 0;
...
Return ONLY the HTML code, no markdown formatting, no explanations.`;
        logger_1.logger.info('[Template Generation Service] Calling OpenAI for template HTML generation...');
        const templateStartTime = Date.now();
        const templateCompletionParams = {
            model,
        ...
        const templateModelUsed = templateCompletion.model || model;
        logger_1.logger.info('[Template Generation Service] Template HTML generation completed', {
        ...
        if (templateUsage) {
            ...
            const costData = (0, costService_1.calculateOpenAICost)(templateModelUsed, inputTokens, outputTokens);
            usageInfo = {
                service_type: 'openai_template_generate',
                model: templateModelUsed,
            ...
            await this.storeUsageRecord(tenantId, 'openai_template_generate', templateModelUsed, inputTokens, outputTokens, costData.cost_usd, jobId);
        }
...
Return JSON format: {"name": "...", "description": "..."}`;
        logger_1.logger.info('[Template Generation Service] Calling OpenAI for template name/description generation...');
        const templateNameStartTime = Date.now();
        const templateNameCompletionParams = {
            model,
        ...
        const templateNameModel = templateNameCompletion.model || model;
        logger_1.logger.info('[Template Generation Service] Template name/description generation completed', {
        ...
        if (templateNameUsage) {
            ...
            const costData = (0, costService_1.calculateOpenAICost)(templateNameModel, inputTokens, outputTokens);
            usageInfo = {
                service_type: 'openai_template_generate',
                model: templateNameModel,
                input_tokens: inputTokens,
            ...
            await this.storeUsageRecord(tenantId, 'openai_template_generate', templateNameModel, inputTokens, outputTokens, costData.cost_usd, jobId);
        }
        // Validate response has output_text
        ...
        try {
            ...
            logger_1.logger.warn('[Template Generation Service] Failed to parse template name JSON, using defaults', { error: e });
        }
        return { templateName, templateDescription, usageInfo };
    }
}
exports.TemplateGenerationService = TemplateGenerationService;
//# sourceMappingURL=templateGenerationService.js.map