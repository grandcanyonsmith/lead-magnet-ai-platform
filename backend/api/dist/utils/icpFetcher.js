...
exports.fetchICPContent = fetchICPContent;
exports.buildBrandContext = buildBrandContext;
 ...
 * ```
 */
async function fetchICPContent(url) {
    if (!url || typeof url !== 'string' || url.trim().length === 0) {
        logger_1.logger.warn('[ICP Fetcher] Empty or invalid URL provided');
        return null;
    }
    try {
        (0, validators_1.validateUrl)(url, 'ICP document URL');
    }
    catch (error) {
        logger_1.logger.warn('[ICP Fetcher] Invalid URL format', {
            url,
    ...
    return (0, retry_1.retryWithBackoff)(async () => {
        logger_1.logger.info('[ICP Fetcher] Fetching ICP document', { url });
        ...
        const response = await (0, timeout_1.withTimeout)(fetchPromise, FETCH_TIMEOUT, `ICP fetch timed out after ${FETCH_TIMEOUT}ms`);
        if (!response.ok) {
            logger_1.logger.warn('[ICP Fetcher] Failed to fetch ICP document', {
                url,
        ...
        if (content.length > MAX_CONTENT_LENGTH) {
            logger_1.logger.warn('[ICP Fetcher] ICP content truncated', {
                url,
        ...
        logger_1.logger.info('[ICP Fetcher] Successfully fetched ICP document', {
            url,
        ...
        onRetry: (attempt, error) => {
            logger_1.logger.debug('[ICP Fetcher] Retrying fetch', {
                attempt,
                url,
                error: error instanceof Error ? error.message : String(error),
            });
        ...
        logger_1.logger.error('[ICP Fetcher] Failed to fetch ICP document after retries', {
            url,
            error: error instanceof Error ? error.message : String(error),
        });
        return null;
 ...
 */
async function processContent(response, contentType, url) {
    if (contentType.includes('application/json')) {
        try {
            ...
            logger_1.logger.warn('[ICP Fetcher] Failed to parse JSON content', {
                url,
                error: error instanceof Error ? error.message : String(error),
            });
            return null;
    ...
    if (contentType.includes('application/pdf')) {
        logger_1.logger.warn('[ICP Fetcher] PDF content type not supported', { url, contentType });
        return null;
    }
    ...
    try {
        ...
        logger_1.logger.warn('[ICP Fetcher] Failed to extract text content', {
            url,
            contentType,
            error: error instanceof Error ? error.message : String(error),
        });
...
//# sourceMappingURL=icpFetcher.js.map