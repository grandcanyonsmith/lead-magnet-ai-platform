...
exports.detectParallelOpportunities = detectParallelOpportunities;
exports.resolveExecutionGroups = resolveExecutionGroups;
exports.validateDependencies = validateDependencies;
exports.getReadySteps = getReadySteps;
...
function buildDependencyGraph(steps) {
    ...
    steps.forEach((step, index) => {
        if (step.depends_on && Array.isArray(step.depends_on)) {
            ...
            const deps = step.depends_on.filter(depIndex => depIndex >= 0 && depIndex < steps.length && depIndex !== index);
            ...
            const lowerOrderSteps = steps
                .map((s, i) => ({ step: s, index: i, order: s.step_order !== undefined ? s.step_order : i }))
 ...
 * ```
 */
function detectParallelOpportunities(steps) {
...
function resolveExecutionGroups(steps) {
    ...
    while (completed.size < steps.length) {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            const allDepsCompleted = deps.length === 0 || deps.every(depIndex => completed.has(depIndex));
            if (allDepsCompleted) {
                readySteps.push(i);
            }
        }
        if (readySteps.length === 0) {
            ...
            console.warn('No ready steps found, but not all steps completed. Possible circular dependency.');
            break;
        }
        // Check if steps in this group can run in parallel
        // Steps can run in parallel if they don't depend on each other
        const canRunInParallel = readySteps.length > 1 && !hasInternalDependencies(readySteps, graph);
 ...
 */
function hasInternalDependencies(stepIndices, graph) {
 ...
 * ```
 */
function getReadySteps(completedStepIndices, allSteps) {
 ...
 * ```
 */
function getStepStatus(completedStepIndices, runningStepIndices, allSteps) {
