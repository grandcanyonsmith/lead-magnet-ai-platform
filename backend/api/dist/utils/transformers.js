 ...
 * ```
 */
function normalizeKeys(obj, format = 'camelCase') {
    if (!(0, validators_1.isObject)(obj)) {
        return obj;
    }
    const result = {};
    for (const [key, value] of Object.entries(obj)) {
        ...
        if ((0, validators_1.isObject)(value)) {
            result[normalizedKey] = normalizeKeys(value, format);
        }
        else if ((0, validators_1.isArray)(value)) {
            result[normalizedKey] = value.map(item => (0, validators_1.isObject)(item) ? normalizeKeys(item, format) : item);
        }
        else {
...
function sanitizeInput(input, options = {}) {
    ...
    if ((0, validators_1.isObject)(input)) {
        const result = {};
    ...
    if ((0, validators_1.isArray)(input)) {
...
function deepMerge(target, ...sources) {
    ...
    for (const source of sources) {
        if (!(0, validators_1.isObject)(source)) {
            continue;
        }
        for (const key in source) {
            if (Object.prototype.hasOwnProperty.call(source, key)) {
                ...
                if ((0, validators_1.isObject)(sourceValue) && (0, validators_1.isObject)(targetValue)) {
...
function unflatten(obj) {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        for (let i = 0; i < keys.length - 1; i++) {
            const k = keys[i];
            if (!(k in current) || !(0, validators_1.isObject)(current[k])) {
                current[k] = {};
            }
            current = current[k];
        }
