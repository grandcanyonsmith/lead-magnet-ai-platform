...
class ErrorHandler:
    """Handles OpenAI API error classification and standardized exception creation."""
    ...
    def create_error_exception(
        ...
        if error_category == "authentication":
            ...
            logger.warning(f"[ErrorHandler] Rate limit exceeded - request should be retried")
            ...
            logger.error(f"[ErrorHandler] Invalid tool_choice configuration - tool_choice='required' but tools empty", extra={
            ...
            logger.error(f"[ErrorHandler] Invalid model specified: {model}")
            return Exception(f"Invalid AI model specified. Please check your workflow configuration: {error_message}")
        elif error_category == "timeout":
            logger.warning(f"[ErrorHandler] Request timeout - request took too long")
            ...
            logger.error(f"[ErrorHandler] Connection error - network issue")
            return Exception(f"Unable to connect to OpenAI API. Please check your network connection: {error_message}")
        else:
            logger.error(f"[ErrorHandler] Unexpected API error: {error_type}")
