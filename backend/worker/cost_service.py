...
OPENAI_PRICING = {
    ...
    # Treat GPT-5.2 as the same pricing tier as GPT-5 unless updated pricing is provided
    'gpt-5.2': {
        'input_per_1k_tokens_usd': Decimal('0.005'),
        'output_per_1k_tokens_usd': Decimal('0.015'),
    },
...
def calculate_openai_cost(model: str, input_tokens: int, output_tokens: int) -> Dict[str, float]:
    ...
    """
    pricing = OPENAI_PRICING.get(model, OPENAI_PRICING['gpt-5.2'])  # Default to gpt-5.2 if unknown
