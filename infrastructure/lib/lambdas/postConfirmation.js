 ...
 *
 * Responsibilities:
 * - Create a customer record (leadmagnet-customers) if missing
 * - Create a user record (leadmagnet-users) if missing
 * - Persist `custom:customer_id` and `custom:role` back to Cognito via AdminUpdateUserAttributes
 *
 * Notes:
 * - Stripe customer creation is intentionally deferred to the billing flow (backend does lazy creation).
 * - This lambda must be safe to re-run (idempotent-ish).
 */
...
function pickCustomerId(attrs, username) {
  return (
    attrs['custom:customer_id'] ||
    attrs['custom:tenant_id'] ||
    username ||
    crypto.randomUUID()
  );
}
...
exports.handler = async (event) => {
  ...
  if (!usersTable || !customersTable) {
    throw new Error('Missing USERS_TABLE or CUSTOMERS_TABLE environment variables');
  }

  const userPoolId = event.userPoolId;
  const username = event.userName; // Cognito user sub (UUID-like)
  const attrs = (event.request && event.request.userAttributes) || {};

  const email = attrs.email || attrs['custom:email'] || undefined;
  const name = attrs.name || attrs.given_name || email || 'Customer';
  const customerId = pickCustomerId(attrs, username);

  const ts = nowIso();

  // Create customer record if missing (do not overwrite existing).
  try {
    await ddb
      .put({
  ...
  // Create user record if missing (do not overwrite existing).
  try {
    await ddb
      .put({
  ...
  // Ensure Cognito claims include customer_id/role for subsequent JWTs.
  const updates = [];
  if (!attrs['custom:customer_id']) {
    updates.push({ Name: 'custom:customer_id', Value: customerId });
  }
  if (!attrs['custom:role']) {
    updates.push({ Name: 'custom:role', Value: 'USER' });
  }
